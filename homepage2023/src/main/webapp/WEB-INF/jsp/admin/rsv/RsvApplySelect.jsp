...
<div id="content">
   <div id="contents">
       <form action="/admin/rsv/rsvApplyConfirm.do" method="post" id="frm" onsubmit="return regist()">
           ...
           <input type="hidden" name="reqstId" value="${result.reqstId}"/>
            ...
            <table class="chart2">
               ...
               <tbody>
                  ...
                  <tr id="resn" <c:if test="${result.confmSeCode ne 'X'}">style="display:none;"</c:if>>
                      <th>반려사유</th>
                      <td>
                         <textarea rows="5"  id="returnRean" name="returnRean"><c:out value="${result.returnRean}" /></textarea>
                      </td>
                  </tr>
               </tbody>  
            </table>
            <div class="btn-cont ar">
                <c:choose>
                    ...
                    <c:otherwise>
                       <a href="#none" id="btn-req" class="btn spot">등록</a>
                    </c:otherwise>
                </c:choose>
...
$(document).ready(function(){
   ...
   $("#confmSeCode").change(function(){
	...
	//vali
	function regist(){
		if($("#confmSeCode").val() == "X" && !$("#returnRean").val()){
			alert("반려 사유를 작성해주세요.");
			$("#returnRean").focus();
			return false;
		}
